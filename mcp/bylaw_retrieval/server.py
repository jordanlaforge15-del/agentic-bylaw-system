...
def create_mcp_server(db_url: str | None = None):
    try:
...
def run_stdio(db_url: str | None = None) -> None:
    create_mcp_server(db_url).run()


def run_streamable_http(db_url: str | None = None) -> None:
    create_mcp_server(db_url).run(transport="streamable-http")
...
def main() -> None:
    ...
    if args.http:
        run_streamable_http(args.db_url)
        return
    run_stdio(args.db_url)
