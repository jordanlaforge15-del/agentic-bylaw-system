...
const ParcelContent = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, height: '100%' }}>
      ...
      <div style={{ padding: '12px 20px', borderTop: `1px solid ${t.hair}`, display: 'flex', flexDirection: 'column', gap: 8, flexShrink: 0 }}>
...
const RightPane = () => {
  ...
  const sourceCount = (typeof CITATIONS_DATA !== 'undefined' && CITATIONS_DATA.shown_matches) || 4;

  return (
    <aside style={{ width: 340, borderLeft: `1px solid ${t.hair}`, background: t.surfaceAlt, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      {/* Tab strip */}
      <div style={{ display: 'flex', borderBottom: `1px solid ${t.hair}`, background: t.surface, flexShrink: 0 }}>
        {[
          ...
          return (
            <button key={it.id} onClick={() => setTab(it.id)} style={{
              ...
              <span>{it.label}</span>
              {it.count != null && (
                <span style={{ fontFamily: 'JetBrains Mono', fontSize: 10, letterSpacing: '0.04em', color: t.textMuted }}>· {it.count}</span>
              )}
            </button>
          );
        })}
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        {tab === 'sources' ? <CitationsPane /> : <ParcelContent />}
      </div>
    </aside>
  );
};
