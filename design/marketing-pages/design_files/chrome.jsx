...
const Footer = ({ setRoute }) => {
  ...
  const cols = [
    { h: 'Product', items: [
      ...
      { label: 'Coverage', r: 'coverage' },
    ]},
    { h: 'Account', items: [
      { label: 'Log in', r: 'login' },
      { label: 'Get an invite', r: 'signup' },
      { label: 'Billing', r: 'billing' },
      { label: 'Support', r: 'support' },
    ]},
    { h: 'Company', items: [
      { label: 'About', r: 'about' },
      { label: 'Privacy', r: 'privacy' },
      { label: 'Terms', r: 'terms' },
        ...
        {cols.map(c => (
          <div key={c.h} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {c.items.map(i => i.href ? (
              <a key={i.label} href={i.href} style={{ fontSize: 13, color: t.text, textDecoration: 'none' }}>{i.label}</a>
            ) : (
              <button key={i.label} onClick={() => setRoute && setRoute(i.r)} style={{ background: 'none', border: 'none', padding: 0, fontFamily: 'inherit', fontSize: 13, color: t.text, cursor: 'pointer', textAlign: 'left' }}>{i.label}</button>
            ))}
          </div>
        ))}
      </div>
