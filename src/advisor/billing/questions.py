...
QUESTION_DEVELOPMENT_STANDARDS_DEF = Question(
    ...
    prompt_template=(
        ...
        "FAR, parking). Project details: {project_details}. "
        "BEGIN with a mandatory 'Use permission' section, BEFORE any "
        "built-form standard: resolve the proposed use against the zone and "
        "state whether it is permitted as-of-right, permitted only "
        "conditionally, or prohibited, citing the governing by-law provision. "
        "Use permission is the go/no-go gate for the whole proposal — if the "
        "use itself is not allowed, the built-form compliance is moot, so "
        "lead with it. Where a use is only conditionally permitted (e.g. a "
        "multi-unit dwelling in the INS zone is conditionally permitted only "
        "on a Schedule 9 landmark site under Section 43), state that "
        "condition explicitly as the decisive threshold. If use permission "
        "cannot be resolved because a required dataset is absent from the "
        "corpus (e.g. Schedule 9 landmark designation), say so as an explicit "
        "'cannot resolve — Schedule 9 absent from corpus' row citing the "
        "ingestion-gap doc (docs/data-gaps/schedule-9-18-ingestion.md) — "
        "never silently omit the use-permission evaluation. "
        "THEN list each built-form standard, the proposed value, and "
        "pass/fail with the governing by-law provision. Where a side or rear "
        "setback requirement is conditional on the zone of an abutting "
        "property, call get_adjacent_zoning to resolve the neighbouring "
        "parcels' zoning and give a DEFINITIVE pass/fail for that standard — "
        "do not leave it as 'uncertain — depends on adjacent zoning'. If a "
        "governing figure genuinely cannot be resolved from the data, name "
        "the specific schedule or dataset that would contain it rather than "
        "defaulting the verdict to 'verify with HRM'."
    ),
    catalog_anchor=(
