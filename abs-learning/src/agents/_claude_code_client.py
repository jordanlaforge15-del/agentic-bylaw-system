...
class ClaudeCodeClient:
    ...
    def _invoke_with_retry(
        ...
        """Call ``claude -p`` up to ``self.max_retries`` times. Each attempt
        either returns a parsed structured_output dict or raises. After
        ``self.max_retries`` consecutive failures, raises ClaudeCodeBackendError
        with the last failure message — NEVER falls back to the API path.
        """
        last_error: str | None = None
        for attempt in range(1, self.max_retries + 1):
            try:
                return self._invoke_once(prompt, schema, attempt=attempt)
            except ClaudeCodeBackendError as exc:
                last_error = str(exc)
                logger.warning(
                    "ClaudeCodeClient attempt %d/%d failed: %s",
                    attempt, self.max_retries, exc,
                )
                if attempt < self.max_retries:
                    # Repair-mode prompt for the next attempt: append the
        ...
        raise ClaudeCodeBackendError(
            f"Claude Code backend failed after {self.max_retries} attempts. "
            f"Last error: {last_error}"
        )

    def _invoke_once(
        self, prompt: str, schema: dict[str, Any], *, attempt: int,
    ) -> dict[str, Any]:
        """Single subprocess invocation. Returns parsed structured_output
        on success, raises ClaudeCodeBackendError otherwise.
        """
        cmd = [
        ...
        try:
            ...
            raise ClaudeCodeBackendError(
                f"`claude -p` timed out after {self.timeout_s}s (attempt {attempt})"
            ) from exc
