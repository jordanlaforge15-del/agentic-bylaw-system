...
test.describe("message feedback", () => {
  test("thumbs up/down buttons appear and toggle on agent messages", async ({
    page,
  }) => {
  ...
  test("flag panel opens, requires reason, and submits", async ({ page }) => {
    const { caseId } = await openCaseViaApi();
    await page.goto(`/app?case_id=${caseId}`);
  ...
  test("cancel closes flag panel without submitting", async ({ page }) => {
    const { caseId } = await openCaseViaApi();
    await page.goto(`/app?case_id=${caseId}`);
