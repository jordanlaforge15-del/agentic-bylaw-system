...
test("general feedback: thanks message auto-dismisses after 4 seconds", async ({
  page,
}) => {
  await page.goto("/app");
  ...
  await page
    .getByTestId("general-feedback-message")
    .fill("The sidebar layout is confusing.");
  ...
  // Thanks message should be gone and feedback button should reappear
  await expect(page.getByTestId("general-feedback-thanks")).not.toBeVisible();
  await expect(page.getByTestId("general-feedback-open")).toBeVisible();
});

test("general feedback: can submit multiple times after auto-dismiss", async ({
  page,
}) => {
  await page.goto("/app");
  ...
  // First submission
  await page.getByTestId("general-feedback-open").click();
  await page.getByTestId("feedback-category-general_satisfaction").click();
  await page.getByTestId("general-feedback-message").fill("First feedback");
  await page.getByTestId("general-feedback-submit").click();
  ...
  await page.getByTestId("general-feedback-message").fill("Second feedback");
  await page.getByTestId("general-feedback-submit").click();

  // Thanks message should appear again
  await expect(page.getByTestId("general-feedback-thanks")).toBeVisible();
});
