...
* A ``Halifax Regional Municipality / Regional Centre Land Use By-Law (POCS
  Schedule 7 E2E)`` document with a ``Schedule 7`` fragment (p.27
  list-of-schedules reference) so the dataset linker binds the overlay to its
  citing fragment by ``citation_label``. The document identity is deliberately
  unique (not the real "HRM / Regional Centre Land Use By-Law" name) so it does
  NOT collide with ``seed_e2e_address_profile.py``'s document in the shared
  e2e DB — the linker matches documents by ``(municipality, bylaw_name)`` only.
...
# The e2e corpus stuffs many synthetic bylaws sharing names into one shared
# `layer1_test` DB, and `link_dataset_to_bylaw` resolves the citing document by
# `(municipality, bylaw_name)` ALONE. Reusing "HRM / Regional Centre Land Use
# By-Law" here (the real prod name, correctly used by the production config +
# unit tests) collided with `seed_e2e_address_profile.py`'s document: the
# ambiguous match linked the address-profile ZONE dataset to THIS document —
# which has only a Schedule 7 fragment, no Zoning Schedule — so the zone
# overlay went unlinked and `get_address_profile` returned zone=null,
# regressing abs274-bylaw-query + address-profile-mcp-tool. Give the e2e seed a
# unique document identity so its `document_match` resolves to itself only,
# exactly as `seed_e2e_zone_profile.py` does with its "(Zone Profile E2E)" name.
DOCUMENT_MUNICIPALITY = "Halifax Regional Municipality"
DOCUMENT_BYLAW_NAME = "Regional Centre Land Use By-Law (POCS Schedule 7 E2E)"
...
def _get_or_create_document(session) -> Document:
    ...
    if document is not None:
        # The shared `layer1_test` volume persists across `make e2e` runs, so a
        # document written by an EARLIER version of this seed (which reused the
        # real "HRM / Regional Centre Land Use By-Law" name) survives with its
        # stale colliding identity. `_get_or_create_document` keys on file_hash,
        # so refresh the identity fields to the current unique values — otherwise
        # the collision that regressed the address-profile zone link never clears
        # on a persistent DB and this dataset's own document_match misses.
        document.municipality = DOCUMENT_MUNICIPALITY
